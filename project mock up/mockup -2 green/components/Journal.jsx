...
const JournalSection = ({ navigate }) => {
  const [view, setView] = React.useState('list'); // list | post | editor
  ...
  const openPost = (entry) => { setActivePost(entry); setView('post'); };
      ...
      <div style={{ maxWidth: '740px', margin: '0 auto' }}>
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 300, lineHeight: 1.85, color: 'var(--text-200)' }}>
          ...
          <blockquote style={{ borderLeft: '3px solid var(--gold-400)', paddingLeft: '28px', margin: '40px 0', fontStyle: 'italic', fontSize: '24px', color: 'var(--gold-400)', lineHeight: 1.5 }}>
            "The gains are invisible at first. Then, one day, they are not."
          </blockquote>
    ...
    <div style={{ maxWidth: '1100px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {tags.map(t => (
            <button key={t} onClick={() => setActiveTag(t)} style={{ padding: '6px 14px', background: activeTag === t ? 'var(--gold-400)' : 'transparent', border: `1px solid ${activeTag === t ? 'var(--gold-400)' : 'var(--glass-border)'}`, color: activeTag === t ? 'oklch(0.08 0.018 258)' : 'var(--text-300)', borderRadius: '20px', fontFamily: 'var(--font-ui)', fontSize: '11px', fontWeight: activeTag === t ? 600 : 400, cursor: 'pointer', transition: 'all 0.15s' }}>{t}</button>
          ))}
        </div>
        <button onClick={() => { setActiveTag('All'); setView('editor'); }} style={{ padding: '9px 20px', background: 'linear-gradient(135deg, var(--gold-400), var(--gold-500))', border: 'none', color: 'oklch(0.08 0.018 258)', borderRadius: '6px', fontFamily: 'var(--font-ui)', fontSize: '12px', fontWeight: 600, cursor: 'pointer', letterSpacing: '0.06em' }}>
          + New entry
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '14px' }}>
        {rest.map(entry => (
          <div key={entry.id} className="glass" onClick={() => openPost(entry)} style={{ padding: '26px', cursor: 'pointer', transition: 'transform 0.2s, border-color 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.borderColor = 'oklch(0.35 0.008 258 / 0.55)'; }}
